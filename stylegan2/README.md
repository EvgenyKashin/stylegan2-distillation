...
python run_generator.py generate-images-custom --network=gdrive:networks/stylegan2-ffhq-config-f.pk \
  --truncation-psi=0.7 --num 5000 --result-dir generated_faces
```

### Generate with a vector
```
python run_generator.py generate-images-custom --network=gdrive:networks/stylegan2-ffhq-config-f.pkl \
 --truncation-psi=0.7 --num 50000 --result-dir generated_ffhq_smile \
 --direction_path stylegan2directions/smile.npy --coeff 1.5
```

### Apply a vector after backprop
```
python run_generator.py apply-vector-on-backprop --network=gdrive:networks/stylegan2-ffhq-config-f.pkl \
  --dlatents_files_pattern results/00007-project-real-images/ --direction_path m2f_3.txt
```
